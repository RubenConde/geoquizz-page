    ...
    <div class="card">
        ...
        <div class="card-content" v-if="showCard">
            <b-field group-multiline grouped>
                ...
                <div class="control is-flex">
                    <b-switch class="is-hidden-tablet" size="is-small" v-model="hasMobileCards">Mobile cards</b-switch>
                </div>
            </b-field>
            <b-table :data="games" :mobile-cards="hasMobileCards" :paginated="isPaginated" :per-page="perPage"
                     :show-detail-icon="false" aria-current-label="Current page" aria-next-label="Next page"
                     aria-page-label="Page" aria-previous-label="Previous page" detailed hoverable narrowed ref="table"
                     sortable striped>
                <template slot-scope="props">
                    ...
                    <b-table-column centered field="game.status" label="Started" numeric sortable>
                        {{ new Date(props.row.game.created_at).toLocaleString() }}
                    </b-table-column>
                    <b-table-column centered field="game.status" label="Finalized" numeric sortable>
                        {{ (props.row.game.updated_at === props.row.game.created_at) ? '-' : new
                        Date(props.row.game.updated_at).toLocaleString()}}
                    </b-table-column>
                </template>
                <template slot="empty">
