...
export default {
   ...
   methods: {
      async modifDiff() {
         ...
         await this.$store.dispatch('UPDATE_DIFFICULTY', diffUpdated).then(async (response) => {
            if (response.success) {
               await this.getInfo();
      ...
      confirmCustomDelete() {
         Dialog.confirm({
            ...
            message: 'Are you sure you want to <b>delete</b> this difficulty? This action cannot be undone.',
      ...
      async delDiff() {
         await this.$store.dispatch('DELETE_DIFFICULTY', this.selectedDiff.id).then(async () => {
            await this.getInfo();
