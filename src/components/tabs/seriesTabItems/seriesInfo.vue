<template>
  <div class="modal-card">
    <header class="modal-card-head is-small">
      ...
      <b-tooltip
        :label="'This place has ' + selectedSeries.games.length + ' game(s)'"
        position="is-left"
      >
    ...
    <footer class="modal-card-foot">
      <b-button @click="$parent.close()" size="is-small" type="is-danger"
        >Cancel</b-button
      >
      <b-button @click="modifSeries" size="is-small" type="is-success"
...
export default {
  ...
  methods: {
    ...
    async modifSeries() {
      ...
      await this.$store
        .dispatch("UPDATE_SERIES", seriesUpdated)
        .then(async response => {
          if (response.success) {
            await this.getInfo();
