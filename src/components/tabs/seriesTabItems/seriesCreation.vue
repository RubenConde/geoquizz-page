        ...
        <section class="card-content" v-if="showCard">
            <div class="columns is-multiline">
                <div class="column is-full">
                    <gmap-autocomplete :value="newSeries.city" @place_changed="setPlace" class="input is-small">
                    </gmap-autocomplete>
                </div>
                <div class="column is-full">
                    <gmap-map :center="{lat : newSeries.latitude, lng : newSeries.longitude}" :options="mapOptions"
                              :zoom="newSeries.zoom" @zoom_changed="changeZoomLevel" class="map">
                        <GmapMarker :draggable="true" :position="{lat : newSeries.latitude, lng : newSeries.longitude}"
                                    @dragend="changeMarkerPosition"/>
                    </gmap-map>
                </div>
                <b-button @click="createNewSeries" size="is-small" type="is-success">Create</b-button>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            async createNewSeries() {
                ...
                await this.$store.dispatch('CREATE_SERIES', this.newSeries)
                    .then(async response => {
                        if (response.success) {
                            await this.getInfo();
