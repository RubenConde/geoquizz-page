  ...
  <div class="modal-card">
    ...
    <section class="modal-card-body">
      ...
      <b-field label="Series">
        <b-select expanded v-model="photoChanged.idSeries">
          <option
            :key="aSeries.series.id"
            :value="aSeries.series.id"
    ...
    <footer class="modal-card-foot">
      <b-button @click="$parent.close()" size="is-small" type="is-danger"
        >Cancel</b-button
      >
      <b-button @click="modifPhoto" size="is-small" type="is-success"
...
export default {
  ...
  methods: {
    ...
    async modifPhoto() {
      ...
      await this.$store
        .dispatch("UPDATE_PHOTO", photoUpdated)
        .then(async response => {
          if (response.success) {
            await this.getInfo();
    ...
    confirmCustomDelete() {
      this.$dialog.confirm({
        ...
        message:
          "Are you sure you want to <b>delete</b> this photo? This action cannot be undone.",
    ...
    async delPhoto() {
      await this.$store
        .dispatch("DELETE_PHOTO", this.selectedPhoto.id)
        .then(async () => {
          await this.getInfo();
