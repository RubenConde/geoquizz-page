<template>
    ...
    <div class="modal-card">
        ...
        <section class="modal-card-body">
            ...
            <b-field>
                <gmap-map :center="{lat : photoChanged.latitude, lng : photoChanged.longitude}" :options="mapOptions"
                          :zoom="15" class="map">
                    <GmapMarker :draggable="true"
                                :position="{lat : photoChanged.latitude, lng : photoChanged.longitude}"
                                @dragend="changeMarkerPosition"/>
                </gmap-map>
            </b-field>
            <b-field>
                <b-button @click="confirmCustomDelete" class="is-pulled-right" icon-right="delete" type="is-danger">
                    Delete
                </b-button>
            </b-field>
        </section>
    ...
    export default {
        ...
        methods: {
            ...
            async modifPhoto() {
                ...
                await this.$store.dispatch('UPDATE_PHOTO', photoUpdated)
                    .then(async response => {
                        if (response.success) {
                            await this.getInfo();
            ...
            async delPhoto() {
                await this.$store.dispatch('DELETE_PHOTO', this.selectedPhoto.id)
                    .then(async () => {
                        await this.getInfo();
