...
const getters = {
   DIFFICULTY_BY_ID: (state, getters) => (idDiff) => {
      return state.difficulties.filter((diff) => diff.difficulty.id === idDiff);
   },
};
const mutations = {
...
const actions = {
   GET_DIFFICULTIES: async (context, payload) => {
      let data;
      await DifficultyRepository.get(payload).then((response) => {
         if (response.data.success) context.commit('SET_DIFFICULTIES', response.data.data.difficulties);
         data = response.data;
      });
      return data;
   },
   GET_DIFFICULTY: async (context, payload) => {
      let data;
      await DifficultyRepository.getDifficulty(payload).then((response) => {
         if (response.data.success) context.commit('SET_DIFFICULTY', response.data.data.difficulty);
         data = response.data;
      });
      return data;
   },
   CREATE_DIFFICULTY: async (context, payload) => {
      let data;
      await DifficultyRepository.createDifficulty(payload).then((response) => {
         if (response.data.success) context.commit('ADD_DIFFICULTY', response.data.data);
         data = response.data;
      });
      return data;
   },
